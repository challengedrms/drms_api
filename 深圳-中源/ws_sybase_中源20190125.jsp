<%@ page contentType="text/html;charset=UTF-8" import="org.directwebremoting.json.JsonUtil,com.openjweb.b2c.entity.*,com.openjweb.alipay.sign.MD5,net.sf.json.*,org.openjweb.core.service.*,org.openjweb.core.entity.*,org.openjweb.core.util.*,org.openjweb.core.eai.util.*,java.util.*,javax.naming.*,javax.sql.*,java.sql.*,java.text.*,java.io.*,org.springframework.web.multipart.MultipartFile,org.springframework.web.multipart.MultipartHttpServletRequest,org.springframework.web.multipart.commons.CommonsMultipartResolver"%><%
	...
	//uf_task_upload_01_00011 - 同步数据库端时间
	if ("01-00011".equals(act)) 
	{
		reMsg = this.getDataBaseTime2();
	}
    ...
    {
    	reMsg = this.getMemPotQuery(param,brId);
    }
...
/*
			Object[] parmsEdiH = new Object[15];
			parmsEdiH[0]	= JO_orderList_row.get("orderCode");				//edih_voucher_id	订单唯一标识号
			parmsEdiH[1]	= "ZG";												//edih_type
			parmsEdiH[2]	= JO_orderList_row.get("orgCode");					//edih_org_id
			parmsEdiH[3]	= parseToDate(JO_orderList_row.get("postTime"));	//edih_tx_date	DATE
			parmsEdiH[4]	= JO_orderList_row.get("orderId");					//edih_ref_id1	89-订单唯一流水号
			parmsEdiH[5]	= JO_orderList_row.get("userId");					//edih_ref_id2	用户(客户)ID
			parmsEdiH[6]	= JO_orderList_row.get("tel");						//edih_ref_id3	联系电话
			parmsEdiH[7]	= JO_orderList_row.get("orderAmount");				//edih_ref_id4	订单金额
			parmsEdiH[8]	= JO_orderList_row.get("payAmount");				//edih_ref_id5	支付金额
			parmsEdiH[9]	= JO_orderList_row.get("consignee");				//edih_memo		联系人
			parmsEdiH[10]	= "000000";											//edih_input_user		输入人
			parmsEdiH[11]	= parseToDate(JO_orderList_row.get("postTime"));	//edih_input_date		输入日	DATE
			parmsEdiH[12]	= "000000";											///edih_audit_user		审核人
			parmsEdiH[13]	= parseToDate(JO_orderList_row.get("payTime"));		//edih_audit_date		审核日	DATE
			parmsEdiH[14]	= "0";												//edih_flag		
			
			LogUtil.info("edih_type:",	 	parmsEdiH[1].toString(),true);
			LogUtil.info("edih_tx_date:",	 	parmsEdiH[3].toString(),true);

			if(!this.executeUpdate(conn,sqlEdiH,parmsEdiH,false,JO_orderList_row.toString()))
			 {
				 isSuccess = false;
				 break;
			 }
*/			
	...
	private List<Object> insertDataPiv(Connection conn,JSONArray json) throws Exception
	{
		...
		String sql ="insert into pos.p_inv(piv_bill_no,piv_br_id,piv_date,piv_time,piv_emp,piv_zk_amt,piv_ys_amt,piv_rmb_zl,piv_rmb,piv_fe_flag,piv_fe_amt,"
   ...
   private String getPromotionType(String param,String brId)
   {
	...
	private String api_Mkt_QueryPmList(String param,String brId)
	{
		...
		String sql = "select pnh_id,convert(char(10),pnh_start_date,111) as pnh_start_date,convert(char(10),pnh_end_date,111) as pnh_end_date,pnh_pm_code, "
   ...
   private String getPromotionPlan(String param,String brId)
   {
	...
	private String getDataBaseTime()
    {
		...
		String sql = "select convert(char(10),getdate(), 111)||' '||convert(char(100),getdate(), 8) as time from pos.system_var";
		list = this.query(sql);
	...
	//01-00011.Get database time from ws.
	private String getDataBaseTime2()
    {
		Hashtable hst = new Hashtable(); 
		String erCode = "0",erMsg = "Query Data Success.";
		//String sql = "select to_char(sysdate,'yyyy-mm-dd hh24:mi:ss') as time from dual";
   ...
   private String getMemPotQuery(String param,String brId)
   {
	   Hashtable hst = new Hashtable();
	   String erCode = "0",erMsg = "Query Data Success.";
	   try
	   {
		   JSONObject jsonObj =  JSONObject.fromObject(this.getArray(param,"data").get(0));
		   String sql = "select mc_mem_id,mc_table_status,mp_br_id,mp_name,mp_personal_id,to_char(mp_birth,'yyyy/mm/dd') as mp_birth, "
	...
	private String api_Coupon_QueryCouponUseList(String param,String brId)
	{
		...
		String sql = "select ppu_use_type,ppu_use_org,br_name,ppu_use_bill,ppu_use_amt,convert(char(10),ppu_input_date,111) as ppu_input_date,ppu_pay_mode,ppu_shift "
	...
	private String api_Customer_QueryCustomerPointDetails(String param,String brId)
	{
		...
		String sql = "select convert(char(10),mts_tx_date,111) as CreateTime,'消费积分' as ChangeType,convert(char(8),getdate(),112)||'SUM' as RefCode,mts_br_id as ShopCode,br_name as ShopName,"
	...
	private String api_Order_QueryOrder(String param,String brId)
	{
		...
		String sql = "select convert(char(10),piv_date,111) as OrderDate,piv_time as OrderTime,convert(char(8),piv_date,112)||'-'||piv_br_id||'-'||piv_bill_no as OrderCode,piv_br_id as ShopCode,br_name as ShopName,"
	...
	private Connection getConnection()
	{
		...
		String dsName = "java:comp/env/jdbc/sybase";//Name of the data source.
		try
		{
	...
	private List<Map<String, Object>> query(String sql)
	{
		try 
		{
			return JDBCUtil.query("java:comp/env/jdbc/sybase", sql, 0, 0,"sybase", null);
		}
		catch (Exception ex) 
		{
	...
	private List<Map<String, Object>> query(String sql,Object[] parms)
	{
		try 
		{
			return JDBCUtil.query("java:comp/env/jdbc/sybase", sql, 0, 0,"sybase", parms);
		}
		catch (Exception ex) 
		{
			LogUtil.info("Function query: ",WsConstant.DB_QUERY_SQL + ex.toString(),true);
