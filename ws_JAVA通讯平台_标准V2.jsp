<%@ page contentType="text/html;charset=UTF-8" import="org.directwebremoting.json.JsonUtil,com.openjweb.b2c.entity.*,com.openjweb.alipay.sign.MD5,net.sf.json.*,org.openjweb.core.service.*,org.openjweb.core.entity.*,org.openjweb.core.util.*,org.openjweb.core.eai.util.*,java.util.*,javax.naming.*,javax.sql.*,java.sql.*,java.text.*,java.io.*,org.springframework.web.multipart.MultipartFile,org.springframework.web.multipart.MultipartHttpServletRequest,org.springframework.web.multipart.commons.CommonsMultipartResolver"%><%
	...
	//api_com_QueryShopStockProMulti - 3.8.1	KBJ-得成-库存接口-按机构列表+商品列表列库存
	if ("api_com_QueryShopStockProMulti".equals(act)) 
	{
		reMsg = this.api_com_QueryShopStockProMulti(param,brId,proId);
	}
...
%><%!
	...
	private String cz_crm_mem_attr(String param,String brId)
	{
		...
		try
		{
			...
			lsMemOther = String.valueOf(jsonObj.get("memOther"));
			...
			if (StringUtil.isEmpty(lsMemOther))
			{
				liCount	= liCount+1;
			}
		   ...
		   //LogUtil.info("liCount=",String.valueOf(liCount),true);
					...
					else
					{
						...
						else
						{
							...
							else
							{
								sql ="select mc.mc_id as mem_card_id,mc.mc_mem_id as mem_id,mp.mp_name as mem_name,mp.mp_sum_amount as mem_amt,mp.mp_sum_pot as mem_pot,mp.mp_mobile_phone as mp_mobile_phone,mc.mc_class as mem_class,mcl.mcl_name as mem_class_name,mc_table_status as mem_valid_flag,to_char(mp.mp_birth,'yyyy/mm/dd') as mp_birth,mp.mp_discount_flag as mp_discount_flag,mp.mp_personal_id,mp.mp_sex,mp.mp_address,br.br_prior,mp.mp_br_id  "
										+"from pos.mem_card mc,pos.mem_personal mp,pos.mem_class mcl,pos.branch br "
										+"where nvl(mp.mp_other_tel,'-') = ? and mc.mc_mem_id = mp.mp_id and mc.mc_class = mcl.mcl_id and mp.mp_br_id = br.br_id  ";
								list = this.query(sql,new Object[]{lsMemOther});
							}
						}
					}
				}
			...
			else
			{
				...
				sql ="select mc.mc_id as mem_card_id,mc.mc_mem_id as mem_id,mp.mp_name as mem_name,mp.mp_sum_amount as mem_amt,mp.mp_sum_pot as mem_pot,mp.mp_mobile_phone as mp_mobile_phone,mc.mc_class as mem_class,mcl.mcl_name as mem_class_name,mc_table_status as mem_valid_flag,to_char(mp.mp_birth,'yyyy/mm/dd') as mp_birth,mp.mp_discount_flag as mp_discount_flag,mp.mp_personal_id,mp.mp_sex,mp.mp_address,br.br_prior,mp.mp_br_id  "
						+"from pos.mem_card mc,pos.mem_personal mp,pos.mem_class mcl,pos.branch br "
						+"where mc.mc_id like ?  and mp.mp_mobile_phone like ? and nvl(mp.mp_tel,'-') like ? and mp.mp_name like ? and nvl(mp.mp_other_tel,'-') like ? and mc.mc_mem_id = mp.mp_id and mc.mc_class = mcl.mcl_id and mp.mp_br_id = br.br_id  ";
				list = this.query(sql,new Object[]{lsMemNo,lsMemMp,lsMemTel,lsMemName,lsMemOther});
			}
   ...
   //86-1003. get promotion plan. //param="{\"data\":[{\"pm_id\":\"CX16020011\"}]}";
   private String getPromotionPlan2012_old(String dbType,String param,String brId)
   {
	   Hashtable hst = new Hashtable();
	   String erCode = "0",erMsg = "Query Data Success.";
	   try
   ...
   private String getPromotionPlan2012(String dbType,String param,String brId)
   {
	   ...
	   try
	   {
			...
			if("SYBASE".equals(dbType)) 
			{
				sql = "select pm_header.pnh_id as pnh_id,convert(char(10),pnh_start_date,111) as pnh_start_date,convert(char(10),pnh_end_date,111) as pnh_end_date,pnh_pm_code, "
					 +"pm_apply_to,pm_remark,pnh_level,pnh_time_valid,pnh_time_start,pnh_time_end,convert(char(10),pnh_plan_end,111) as pnh_plan_end,pnh_memo, "
					 +"pm_detail.pnh_pro_id,pm_detail.pnh_price,pm_detail.pnh_group,pm_detail.pnh_qty "
					 +"from pos.promotion_header pm_header,pos.promotion_mix,pos.promotion_detail_product pm_detail where pm_header.pnh_id =? and pnh_pm_code = pm_code and pm_header.pnh_id = pm_detail.pnh_id";
			}
			else
			{
				sql = "select pm_header.pnh_id as pnh_id,to_char(pnh_start_date,'yyyy/mm/dd') as pnh_start_date,to_char(pnh_end_date,'yyyy/mm/dd') as pnh_end_date,pnh_pm_code, "
					 +"pm_apply_to,pm_remark,pnh_level,pnh_time_valid,pnh_time_start,pnh_time_end,to_char(pnh_plan_end,'yyyy/mm/dd') as pnh_plan_end,pnh_memo, "
					 +"pm_detail.pnh_pro_id,pm_detail.pnh_price,pm_detail.pnh_group,pm_detail.pnh_qty "
					 +"from pos.promotion_header pm_header,pos.promotion_mix,pos.promotion_detail_product pm_detail where pm_header.pnh_id =? and pnh_pm_code = pm_code and pm_header.pnh_id = pm_detail.pnh_id";
			}
   ...
   private String getMemPotQuery(String dbType,String param,String brId)
   {
	...
	private String api_com_QueryShopStockProMulti(String param,String brId,String proId)
	{
		Hashtable hst = new Hashtable(); 
		String erCode = "0",erMsg = "Query Data Success.";
		String sql;
		...
		int	brRow;
		int proRow;
		sql = "select st_br_id as ShopCode,st_pro_id as ProId,st_on_hand_qty as OnHandQty from pos.store where (";
		...
		//	 LogUtil.info("Function getProduct: ",result[x],true);

		for (brRow=0; brRow<brList.length; brRow++)
		{
			if (brRow > 0)
			{
				sql = sql + " or ";
			...
			sql = sql + "st_br_id = '" + brList[brRow] + "'";
		}
		
		sql = sql + ") and (";

		for (proRow=0; proRow<proList.length; proRow++)
		{
			if (proRow > 0)
			{
				sql = sql + " or ";
			...
			sql = sql + "st_br_id = '" + proList[proRow] + "'";
		}
