...
%><%!
	...
	private String insertRx(String param,String brId) throws Exception
	{
		...
		sql ="INSERT INTO t_prescription (id,prescription_id,outpatient_id,department,"
				+"status,patient,detail_count,date_prescription,date_create)"
				+" values (?,?,?,?,?,?,?,?,?)";
		...
		for(i=0 ; i < dataRxProList.size(); i++)
		{
			...
			try
			{
				...
				parms[7]=parseToDate(dataGoods.get("DateCreated"));   //VARCHAR2
				parms[8]=parseToDate(dataGoods.get("DataModified"));   //VARCHAR2
   ...
   private String getPromotionPlan(String param,String brId)
   {
	...
	private Connection getConnection()
	{
		...
		String dsName = "java:comp/env/jdbc/mysqlzhzy";//Name of the data source.
		try
		{
	...
	private List<Map<String, Object>> query(String sql)
	{
		try 
		{
			return JDBCUtil.query("java:comp/env/jdbc/mysqlzhzy", sql, 0, 0,"sybase", null);
		}
		catch (Exception ex) 
		{
	...
	private List<Map<String, Object>> query(String sql,Object[] parms)
	{
		try 
		{
			return JDBCUtil.query("java:comp/env/jdbc/mysqlzhzy", sql, 0, 0,"sybase", parms);
		}
		catch (Exception ex) 
		{
			LogUtil.info("Function query: ",WsConstant.DB_QUERY_SQL + ex.toString(),true);
