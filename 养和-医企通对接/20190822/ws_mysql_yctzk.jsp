<%@ page contentType="text/html;charset=UTF-8" import="org.directwebremoting.json.JsonUtil,com.openjweb.b2c.entity.*,com.openjweb.alipay.sign.MD5,net.sf.json.*,org.openjweb.core.service.*,org.openjweb.core.entity.*,org.openjweb.core.util.*,org.openjweb.core.eai.util.*,java.util.*,javax.naming.*,javax.sql.*,java.sql.*,java.text.*,java.io.*,org.springframework.web.multipart.MultipartFile,org.springframework.web.multipart.MultipartHttpServletRequest,org.springframework.web.multipart.commons.CommonsMultipartResolver"%><%
	...
	//api_com_SyncCouponInfo - 3.8.1	KBJ-得成-库存接口-优惠券规则同步接口
	if ("api_com_SyncCouponInfo".equals(act)) 
	{
		reMsg = this.api_com_SyncCouponInfo(param,brId);
	}
...
%><%!
	...
	private String api_com_SyncCouponInfo(String param,String brId) throws Exception
	{
		Hashtable hst = new Hashtable(); 
		String erCode = "-1",erMsg = "卡券规则同步到ERP失败！";
		Object[] parms;
		String sql;
		...
		JSONArray dataArray= getArray(param,"data");
		JSONArray dataMemRuleArray= getArray(param,"MemRule");
		JSONArray dataMemCardRuleArray= getArray(param,"MemCardRule");
		JSONArray dataProTierRuleArray= getArray(param,"ProTierRule");
		JSONArray dataPotUseRuleArray= getArray(param,"PotUseRule");
		JSONArray dataProductUseRuleArray= getArray(param,"ProductUseRule");
		JSONArray dataOrgUseRuleArray= getArray(param,"OrgUseRule");
		
		dataObjectAll = JSONObject.fromObject(dataArray.get(0));

		Connection conn = this.getConnection();

		sql ="INSERT INTO pos.crm_dc_card_rule ( crm_dcr_ruleCode,crm_dcr_ruleName, crm_dcr_ruleStartDate,  crm_dcr_ruleEndDate,   "
				+"		crm_dcr_activeFlg,   crm_dcr_voucherType,   crm_dcr_voucherSubType,   crm_dcr_discountType,   crm_dcr_vouRuleValue,   "
				+"		crm_dcr_prodLimited,   crm_dcr_customerType, crm_dcr_limitedAmount,   crm_dcr_limitedBuyNum,   crm_dcr_limitedVoucherNum,   "
				+"		crm_dcr_maxNum,   crm_dcr_discount,   crm_dcr_reductionAmount,   crm_dcr_specialAmount,   crm_dcr_superposition,   "
				+"		crm_dcr_effectiveType,   crm_dcr_availabilityChannel,   crm_dcr_fixedEffectiveBegin,   crm_dcr_fixedEffectiveEnd,   "
				+"		crm_dcr_toBeEffectiveDays,   crm_dcr_effectiveDays,   crm_dcr_productType,   crm_dcr_shopLimits,   crm_dcr_voucherName,   "
				+"		crm_dcr_sumNumLimited,  crm_dcr_remainNum,   crm_dcr_activeShopLimit,   crm_dcr_supportRedemp,   crm_dcr_pointType,   "
				+"		crm_dcr_exchangeChannel,   crm_dcr_description,   crm_dcr_YLZD1,   crm_dcr_YLZD2,   crm_dcr_YLZD3,   crm_dcr_YLZD4,  crm_dcr_YLZD5 ) "
				+"		values ( ?,?, ?,  ?,   ?,   ?,   ?,   ?,   ?,   ?,   ?, ?,   ?,   ?,   ?,   ?,   ?,   ?,   ?,   "
				+"		?,   ?,   ?,   ?,   ?,   ?,   ?,   ?,   ?,   ?,  ?,   ?,   ?,   ?,   ?,   ?,   ?,   ?,   ?,   ?,  ? )";
		...
		parms = new Object[40];
		parms[0]=dataObjectAll.get("ruleCode");					//VARCHAR2(30)
		parms[1]=dataObjectAll.get("ruleName");					//VARCHAR2(200)
		parms[2]=parseToDate(dataObjectAll.get("ruleStartDate"));	//DATE
		parms[3]=parseToDate(dataObjectAll.get("ruleEndDate"));		//DATE
		parms[4]=dataObjectAll.get("activeFlg");					//VARCHAR2(30)
		parms[5]=dataObjectAll.get("voucherType");				//VARCHAR2(30)
		parms[6]=dataObjectAll.get("voucherSubType");			//VARCHAR2(30)
		parms[7]=dataObjectAll.get("discountType");				//VARCHAR2(30)
		parms[8]=dataObjectAll.get("vouRuleValue");				//VARCHAR2(30)
		parms[9]=dataObjectAll.get("prodLimited");				//VARCHAR2(30)
		parms[10]=dataObjectAll.get("customerType");				//VARCHAR2(30)
		parms[11]=dataObjectAll.get("limitedAmount");			//NUMBER(18,2)
		parms[12]=dataObjectAll.get("limitedBuyNum");			//NUMBER(18,2)
		parms[13]=dataObjectAll.get("limitedVoucherNum");		//NUMBER(18,2)
		parms[14]=dataObjectAll.get("maxNum");						//NUMBER(18,2)
		parms[15]=dataObjectAll.get("discount");					//NUMBER(18,2)
		parms[16]=dataObjectAll.get("reductionAmount");			//NUMBER(18,2)
		parms[17]=dataObjectAll.get("specialAmount");			//NUMBER(18,2)
		parms[18]=dataObjectAll.get("superposition");			//VARCHAR2(30)
		parms[19]=dataObjectAll.get("effectiveType");			//VARCHAR2(30)
		parms[20]=dataObjectAll.get("availabilityChannel");	//VARCHAR2(30)
		parms[21]=parseToDate(dataObjectAll.get("fixedEffectiveBegin"));	//DATE
		parms[22]=parseToDate(dataObjectAll.get("fixedEffectiveEnd"));		//DATE
		parms[23]=dataObjectAll.get("toBeEffectiveDays");		//NUMBER(18,2)
		parms[24]=dataObjectAll.get("effectiveDays");			//NUMBER(18,2)
		parms[25]=dataObjectAll.get("productType");				//VARCHAR2(30)
		parms[26]=dataObjectAll.get("shopLimits");				//VARCHAR2(30)
		parms[27]=dataObjectAll.get("voucherName");				//VARCHAR2(255)
		parms[28]=dataObjectAll.get("sumNumLimited");			//NUMBER(18,2)
		parms[29]=dataObjectAll.get("remainNum");					//NUMBER(18,2)
		parms[30]=dataObjectAll.get("activeShopLimit");			//VARCHAR2(1)
		parms[31]=dataObjectAll.get("supportRedemp");			//VARCHAR2(1)
		parms[32]=dataObjectAll.get("pointType");					//VARCHAR2(30)
		parms[33]=dataObjectAll.get("exchangeChannel");			//VARCHAR2(30)
		parms[34]=dataObjectAll.get("description");				//VARCHAR2(250)
		parms[35]=dataObjectAll.get("YLZD1");						//VARCHAR2(30)
		parms[36]=dataObjectAll.get("YLZD2");						//VARCHAR2(30)
		parms[37]=dataObjectAll.get("YLZD3");						//VARCHAR2(30)
		parms[38]=dataObjectAll.get("YLZD4");						//VARCHAR2(30)
		parms[39]=dataObjectAll.get("YLZD5");						//VARCHAR2(30)
		
		boolean flag = this.executeUpdate(conn,sql,parms,false,dataObjectAll.toString());
		if(!flag)
		{
			erCode = "-11";
			erMsg  = "Insert crm_dc_card_rule  Failed.";
			return this.getJson(hst,erCode,erMsg);
		}

		erCode = "0";
		erMsg = "Insert crm_dc_card_rule  Successed.";
		return this.getJson(hst,erCode,erMsg);
	}
