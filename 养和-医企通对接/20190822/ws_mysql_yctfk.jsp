...
%><%!
	...
	private String insertYctsSyncGoods(String param,String brId) throws Exception
	{
		...
		for(int i=0 ; i < dataGoodsArray.size(); i++)
		{
			...
			sqlfind = "select count(*) as cc from sync_goods where SRC_ID = ?"; 
			...
			if(list != null && list.size() > 0)
			{
				...
				if(isValue == 0)
				{
					try
					{
						...
						parms[2]=dataGoods.get("pricereal");   //NUMERIC(12,3)
						parms[3]=dataGoods.get("pricemust");   //NUMERIC(12,3)
				...
				else
				{
					try
					{
						sql ="update sync_goods set STOCK=?,PRICE=?,RENDER_PRICE=?,LAST_MODIFY_DATE=?,CREATE_DATE=? where SRC_ID = ?";
						
						parms = new Object[6];
						parms[0]=dataGoods.get("stockqty");   //NUMERIC(12,3)
						parms[1]=dataGoods.get("pricereal");   //NUMERIC(12,3)
						...
						{
							erCode = "-3";
							erMsg  = "Update YctsSyncGoods  Failed.";
							return this.getJson(hst,erCode,erMsg);
						}
					}
					catch(Exception exshgoods)
