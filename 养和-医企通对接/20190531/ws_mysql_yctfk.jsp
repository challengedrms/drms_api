<%@ page contentType="text/html;charset=UTF-8" import="org.directwebremoting.json.JsonUtil,com.openjweb.b2c.entity.*,com.openjweb.alipay.sign.MD5,net.sf.json.*,org.openjweb.core.service.*,org.openjweb.core.entity.*,org.openjweb.core.util.*,org.openjweb.core.eai.util.*,java.util.*,javax.naming.*,javax.sql.*,java.sql.*,java.text.*,java.io.*,org.springframework.web.multipart.MultipartFile,org.springframework.web.multipart.MultipartHttpServletRequest,org.springframework.web.multipart.commons.CommonsMultipartResolver"%><%
    ...
    if("YCT-KC".equals(act)&&!StringUtil.isEmpty(brId)&&!StringUtil.isEmpty(param))
    {
    	reMsg = this.insertYctsSyncGoods(param, brId);
    }
...
%><%!
	...
	private String insertYctsSyncGoods(String param,String brId) throws Exception
	{
		Hashtable hst = new Hashtable(); 
		String erCode = "0",erMsg = "Query Data Success.";
		boolean flag;
		JSONObject dataGoods;
		JSONObject dataHeader;
		String ls_pro_id,sql;
		String sqlfind;
		Object[] parms;

		JSONArray dataGoodsArray= getArray(param,"dataGoods");
		JSONArray dataHeaderArray= getArray(param,"dataHeader");
		dataHeader  = JSONObject.fromObject(dataHeaderArray.get(0));
		...
		for(int i=0 ; i < dataGoodsArray.size(); i++)
		{
			dataGoods  = JSONObject.fromObject(dataGoodsArray.get(i));
			ls_pro_id=String.valueOf(dataGoods.get("goodno")); 
			sqlfind = "select count(*) as cc from sync_goods where COMP_ID = ? AND SRC_ID = ?"; 
			list = this.query(sqlfind,new Object[]{brId,ls_pro_id});
			
			if(list != null && list.size() > 0)
			{
				int isValue = Integer.parseInt(String.valueOf(list.get(0).get("cc"))) ;
				
				LogUtil.info("product_id - "+ls_pro_id,"row no =" + Integer.toString(i) + ";exist=" + Integer.toString(isValue),true);
				
				if(isValue == 0)
				{
					try
					{
						sql ="insert into sync_goods (COMP_ID,SRC_ID,CLS_ID,IS_RECOMMEND,VIEW_ORDER,SALE_POINT,SALE_CNT,";
						sql = sql + "IS_SALE_NOW,SHIP_TYPE,STOCK,PRICE,RENDER_PRICE,BUY_CNT,PAY_TYPES,IS_ADD_POINT,LAST_MODIFY_DATE,CREATE_DATE) ";
						sql = sql + "values(-1,?,3,0,'0',0,0,0,2,?,?,?,0,'0',0,?,?)";
						
						parms = new Object[6];
						parms[0]=dataGoods.get("goodno");   //NMERIC(10,0)
						parms[1]=dataGoods.get("stockqty");   //NUMERIC(12,3)
						parms[2]=dataGoods.get("pricemust");   //NUMERIC(12,3)
						parms[3]=dataGoods.get("pricereal");   //VARCHAR2
						parms[4]=parseToDate(dataHeader.get("lastmodifydate"));   //DATE
						parms[5]=parseToDate(dataHeader.get("createdate"));   //DATE
						
						LogUtil.info("InsertSQL:",sql,true);
						LogUtil.info("InsertSQL:",String.valueOf(parms[0]),true);
						
						flag = this.executeUpdate(conn,sql,parms,false,dataGoods.toString());
						if(!flag)
						{
							erCode = "-3";
							erMsg  = "Insert insertYctsSyncGoods  Failed.";
							return this.getJson(hst,erCode,erMsg);
						}
					}
				...
				else
				{
					try
					{
						sql ="update sync_goods set STOCK=?,PRICE=?,RENDER_PRICE=?,LAST_MODIFY_DATE=?,CREATE_DATE=? where COMP_ID = ? AND SRC_ID = ?";
						
						parms = new Object[6];
						parms[0]=dataGoods.get("stockqty");   //NUMERIC(12,3)
						parms[1]=dataGoods.get("pricemust");   //NUMERIC(12,3)
						parms[2]=dataGoods.get("pricereal");   //VARCHAR2
						parms[3]=parseToDate(dataHeader.get("lastmodifydate"));   //DATE
						parms[4]=parseToDate(dataHeader.get("createdate"));   //DATE
						parms[5]=dataGoods.get("goodno");   //VARCHAR2
						
						flag = this.executeUpdate(conn,sql,parms,false,dataGoods.toString());
						if(!flag)
						{
							erCode = "-3";
							erMsg  = "Update insertYctsSyncGoods  Failed.";
							return this.getJson(hst,erCode,erMsg);
						}
					}
					catch(Exception exshgoods)
					{
						erCode = "-1";
						erMsg  = "Update insertYctsSyncGoods Failed.";
						LogUtil.info("Function insertYctsSyncGoods: ",WsConstant.WS_SAVE_DATA + exshgoods.toString(),true);
					}
				}
   ...
   private String getPromotionPlan(String param,String brId)
   {
	...
	private Connection getConnection()
	{
		...
		String dsName = "java:comp/env/jdbc/mysqlyctfk";//Name of the data source.
		try
		{
	...
	private List<Map<String, Object>> query(String sql)
	{
		try 
		{
			return JDBCUtil.query("java:comp/env/jdbc/mysqlyctfk", sql, 0, 0,"sybase", null);
		}
		catch (Exception ex) 
		{
	...
	private List<Map<String, Object>> query(String sql,Object[] parms)
	{
		try 
		{
			return JDBCUtil.query("java:comp/env/jdbc/mysqlyctfk", sql, 0, 0,"sybase", parms);
		}
		catch (Exception ex) 
		{
			LogUtil.info("Function query: ",WsConstant.DB_QUERY_SQL + ex.toString(),true);
