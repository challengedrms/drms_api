<%@ page contentType="text/html;charset=UTF-8" import="org.directwebremoting.json.JsonUtil,com.openjweb.b2c.entity.*,com.openjweb.alipay.sign.MD5,net.sf.json.*,org.openjweb.core.service.*,org.openjweb.core.entity.*,org.openjweb.core.util.*,org.openjweb.core.eai.util.*,java.util.*,javax.naming.*,javax.sql.*,java.sql.*,java.text.*,java.io.*,org.springframework.web.multipart.MultipartFile,org.springframework.web.multipart.MultipartHttpServletRequest,org.springframework.web.multipart.commons.CommonsMultipartResolver"%><%
	...
	////实施上前检查
	////1.定好数据库连结 "java:comp/env/jdbc/sybase"	是否完成_____
...
%><%!
	...
	private List<Object> insertDataPiv(Connection conn,JSONArray json) throws Exception
	{
		...
		sql = "delete from pos.yb_p_sale where psl_bill_no = ? AND psl_br_id = ? AND psl_date = ? ";
		...
		sql = "delete from pos.yb_package_sale where ps_bill_no = ? AND ps_br_id = ? AND ps_date = ? ";
		...
		sql = "delete from pos.yb_p_inv where piv_bill_no = ? AND piv_br_id = ? AND piv_date = ? ";
		...
		sql = "delete from pos.yb_p_inv_ext1 where pive_bill_no = ? AND pive_br_id = ? AND pive_date = ? ";
		...
		sql = "delete from pos.yb_p_sale_ext1 where psle_bill_no = ? AND psle_br_id = ? AND psle_date = ? ";
		...
		sql = "insert into pos.yb_p_inv(piv_bill_no,piv_br_id,piv_date,piv_time,piv_emp,piv_zk_amt,piv_ys_amt,piv_rmb_zl,piv_rmb,piv_fe_flag,piv_fe_amt,"
	...
	private boolean insertDataPsl(Connection conn,JSONArray json)
	{
		...
		for(int i=0 ; i < json.size(); i++)
		{
			...
			String sql = "insert into pos.yb_p_sale(psl_bill_no,psl_br_id,psl_date,psl_serial,psl_pro_id,psl_qty,psl_prc2,psl_prc1,psl_flag,psl_amt,psl_zs, "
	...
	private boolean insertDataPs(Connection conn,JSONArray json)
	{
		...
		for(int i=0 ; i < json.size(); i++)
		{
			...
			String sql = "insert into pos.yb_package_sale(ps_br_id,ps_date,ps_bill_no,ps_serial,ps_pro_id,ps_qty,ps_type,ps_sale_pro_id) "
					      +" values(?,?,?,?,?,?,?,?)";
	...
	private boolean insertDataPive1(Connection conn,JSONArray json)
	{
		...
		for(int i=0 ; i < json.size(); i++)
		{
			...
			String sql = "insert into pos.yb_p_inv_ext1(pive_bill_no,pive_br_id,pive_date,pive_type,pive_varch1,pive_amt1,pive_amt2, "
	...
	private boolean insertDataPsle1(Connection conn,JSONArray json)
	{
		...
		for(int i=0 ; i < json.size(); i++)
		{
			...
			String sql = "insert into pos.yb_p_sale_ext1(psle_bill_no,psle_br_id,psle_date,psle_serial,psle_type,psle_pro_id,psle_char1, "
	...
	private boolean insertInvProcess(Connection conn,String brId,String billNo,java.sql.Date date)
	{
		...
		String sql = "select count(*) as cc from pos.yb_p_inv_process where pivp_br_id = ? and pivp_bill_no = ? and pivp_date = ?"; 
		...
		if(list != null && list.size() > 0)
		{
			...
			if(isValue == 0)
			{
				sql = "insert into pos.yb_p_inv_process(pivp_bill_no,pivp_br_id,pivp_date,pivp_stock1,pivp_mem1,pivp_bi1,pivp_sync_date) values(?,?,?,?,?,?,?)";
	...
	private String getSalesRecordByStore(String dbType,String param,String brId)
	{
		...
		{
			sql = "select (select count(*) from pos.yb_p_inv where piv_br_id = ? and piv_date = ? and piv_bill_no = ? ) as piv_hq, "
					+"(select count(*) from pos.yb_p_sale where psl_br_id = ? and psl_date = ? and psl_bill_no = ? ) as psl_hq, "
					+"(select  count(*) from pos.yb_package_sale where ps_br_id = ? and ps_date = ? and ps_bill_no  = ? ) as ps_hq, "
					+"(select  count(*) from pos.yb_p_inv_ext1 where pive_br_id  = ? and pive_date = ? and pive_bill_no= ? ) as pive1_hq, "
					+"(select count(*) from pos.yb_p_sale_ext1 where psle_br_id  = ? and psle_date = ? and psle_bill_no= ? ) as psle1_hq  "
					+"from pos.system_var";
		}
		else
		{
   ...
   private String getMemPotQuery(String dbType,String param,String brId)
   {
	...
	private String api_Order_QueryOrder(String dbType,String param,String brId)
	{
		...
		if("SYBASE".equals(dbType)) 
		{
			sql = "select convert(char(10),piv_date,111) as OrderDate,piv_time as OrderTime,convert(char(8),piv_date,112)||'-'||piv_br_id||'-'||piv_bill_no as OrderCode,piv_br_id as ShopCode,br_name as ShopName,"
				...
				+ "'TEST' as InnerMeno,0 as GoodsNum from pos.yb_p_inv,pos.branch "
	...
	private String api_Order_QueryOrderItem(String param,String brId)
	{
		...
		String sql = "select psl_serial as BillRowNo,psl_pro_id as SKU,psl_qty as Qty,psl_batch_num as BatchNum,pro_print_code as ProductNo,"
					...
					+ "from pos.yb_p_sale,pos.product,pos.brand "
